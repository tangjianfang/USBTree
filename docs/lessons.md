...
| L6 | bash 工作目录跨调用残留，相对路径会静默指错位置 | 脚本内一律 `cd "$(dirname "$0")/.."`；交互命令用绝对路径或开头显式 cd；相对链接层级速记——一级目录(30/40/60/70/90)用 `../80-参考资料/`，二级目录(20 的子目录/50/BLE)用 `../../`，三级(BLE 子目录不存在)不存在；已三次踩坑（R36/R49/R53），validate 检查 1 兜底 | 会话内两次 cd 失败 + 三次断链 | 5 |
...
| L8 | evolve #50 发现三份附录被 evolve-log 内容覆盖（R49 的 add -A 提交带入，R44/R45-48/R49 提交前跳过了 validate 所以漏检）；git 基线提交救了命 | 每轮提交前必跑 tools/validate.sh（check 8 的 title≡H1/唯一性会立刻暴露内容覆盖）；大改动用 git add <明确路径> 而非 add -A；出事先 git show <基线>:<file> 对比 | evolve #50 事故 | 1 |
| L9 | 驱动不能凭会话自报输出判定成败（一晚两次实证）：① tail -1 抓到 SessionEnd hook 噪声而非 ROUND 标记——3 个成功轮被误判连续无进展假熔断（01:48）；② 大切片耗尽 max-turns 预算，工作已提交+记账但结论标记被吞——又三次假阴性再次假熔断（02:48） | 标记 grep（`grep -Eo 'ROUND [0-9]+ (DONE\|NOPROGRESS\|BLOCKED)'` 全量取末次）只是快路径；判定以仓库状态为最终事实——无标记时看 evolve-log 指针是否前进+最新轮次行 result 字段定性，指针未动才算会话异常计入熔断；max-turns 预防性上调（60→100）；驱动日志记 marker=[…] 留痕审计 | evolve run3 两次假熔断（2026-09-07 01:48 / 02:48） | 2 |
