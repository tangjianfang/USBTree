...
- **播放（主机→设备）异步模式**：AS 接口额外声明一个同步 IN 反馈端点，UAC1 用 **10.14 定点**报采样率比率（48 kHz 精确主时钟 → **0x0C0000** = 48×2¹⁴，实测在 ±100 ppm 内微调；evolve 勘误——曾误写 0x05A000，该值换算仅 22.5 采样/毫秒）；端点描述符 bRefresh=3 表示 2³ ms 上报一次；
