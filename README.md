...
├── 80-参考资料/               (官方规范原文缓存: 33 份 221MB + 索引 README + 提取文本)
...
- **Graph 回答"知道什么"**：`graph/entities.yaml`（80 个实体：协议/类/机制，带类码与主文档）+ `graph/relations.yaml`（76 条类型化关系边：carries/reuses/supersedes/tunnels/enables…，每条边带 evidence 指向正文）。查询示例："HOGP reuses HID-ReportDescriptor"——跨传输复用、代际更替（BOT→UAS、ECM→NCM、OTG→DRP）、隧道承载（USB4→USB3/PCIe/DP）全部结构化可查；全景图见 `graph/export.mmd`。
