...
- 查官方规范原文 → [80-参考资料/README.md](80-参考资料/README.md)（官方规范本地缓存 + 直链索引；份数以缓存索引页为准）
...
[USB-Labs](https://github.com/tangjianfang/usb-labs) —— 本知识库的**全栈实战配套**：HID/CDC+DFU/MSC/UVC+UAC/PD/BLE 双模/扩展坞七大实验室，从硬件电路设计、固件、主机驱动到应用层与产测工具的商业级实现（"怎么做"在这里，"为什么"在本库）。

## 📄 许可证与规范版权

- 本库原创内容（正文、图谱、技能包、工具脚本）以 [MIT License](LICENSE) 提供；
- `80-参考资料/` 中的官方规范 PDF/ZIP（USB-IF / Bluetooth SIG 版权所有）**不随本仓库分发**（已在 .gitignore 排除，本地缓存不受影响），请通过[缓存索引](80-参考资料/README.md)的官方直链自行下载，并遵守各规范附带许可条款。
